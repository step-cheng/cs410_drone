...
float thrust_neutral = 1350; //1350 for ground effect
...
float y_p_gain = 6;
float y_cam_p_gain = 2;

float auto_thrust = 0;
float thrust_p_gain = 6;
float thrust_d_gain = 0;
float thrust_i_gain = 0;
int z_desired = -800;
int camera_z_last = 0;
...
void compute_motors() {
  ...
  if (camera_data.sequence_num != camera_sequence_last) {
    ...
    thrust_int_error += ((float) z_desired - (float) camera_data.z) * thrust_i_gain;
    thrust_int_error = std::min(std::max(thrust_int_error, (float) - thrust_int_error_sat), (float) thrust_int_error_sat);
    
    auto_thrust = ((float) z_desired - (float) camera_data.z)*thrust_p_gain - ((float) camera_data.z - (float) camera_z_last)/camera_time_diff*thrust_d_gain + thrust_int_error;
    
    camera_z_last = camera_data.z;
    
  }
  
  
  
  float thrust_joystick = thrust_neutral + (128 - (float) joystick_data.thrust) / 128 * thrust_amplitude;
  ...
  if (autonomy_flag) {
    yaw_desired = -(camera_data.yaw * y_cam_p_gain);
    thrust = 0.5*thrust_joystick + 0.5*auto_thrust;
  }
  else {
