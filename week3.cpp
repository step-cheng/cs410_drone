...
float thrust_neutral = 100;
float thrust_amplitude = 100;
float thrust_min = 0;
float thrust_max = 2000;
float pitch_amplitude = 10;
...
void set_motors() {
  ...
  int_p_error = std::min(std::max(int_p_error, (float) -100), (float) 100);
...
int main (int argc, char *argv[])
{
    ...
    while(run_program == 1)
    {
      ...
      flag_print = true;
      if (flag_print) {
        // printf("%f, %f, %f, %d, %d \n", pitch_desired, pitch_filtered, thrust, motor_commands[0], motor_commands[1]);
        printf("%f, %f, %f, %d, %d \n", pitch_filtered, pitch_desired, thrust, motor_commands[0], motor_commands[1]);
